...
.PHONY: up down reset-db seed-sample ingest-month dbt dbt-full-refresh dbt-test ge bench-before bench-after bench-compare explain
...
bench-before:
	...
	$(COMPOSE) run --rm pipeline bench --iters $(BENCH_ITERS) --warmup $(BENCH_WARMUP) --phase before

bench-after:
	$(COMPOSE) exec -T postgres psql -X -U nyc -d nyc_taxi -v ON_ERROR_STOP=1 -f /app/sql/perf/001_create_indexes.sql
	$(COMPOSE) run --rm pipeline bench --iters $(BENCH_ITERS) --warmup $(BENCH_WARMUP) --phase after

bench-compare:
	$(COMPOSE) run --rm pipeline bench-compare
